...
/* local prototypes */
static bool s_CreateWindow(void);
...
static bool s_CreateWindow(void) {
  ...
  window = SDL_CreateWindow(window_title, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, wnd_width, wnd_height,
                            rflags);
  if (!window) {
    DeleteWindow();
    return false;
  ...
  crt.glow = SDL_CreateTexture(renderer, format, SDL_TEXTUREACCESS_STREAMING, wnd_params.width / 2,
                               wnd_params.height / 2);
  ...
  while (SDL_PollEvent(&evt)) {
    switch (evt.type) {
      ...
      case SDL_KEYDOWN:
        ...
        /* special inputs */
        if (keybevt->keysym.sym == SDLK_RETURN && keybevt->keysym.mod & KMOD_ALT) {
          DeleteWindow();
 ...
 */
void
TLN_EnableCRTEffect(TLN_Overlay overlay, uint8_t overlay_factor, uint8_t threshold, uint8_t v0, uint8_t v1, uint8_t v2,
                    uint8_t v3, bool blur, uint8_t glow_factor) {
#pragma EXPORT_FUNC
  int c;
  ...
  SDL_SetHint(SDL_HINT_RENDER_SCALE_QUALITY, "1");
  backbuffer = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STREAMING, wnd_params.width,
                                 wnd_params.height);
 ...
 */
void TLN_DefineInputKey(TLN_Player player, TLN_Input input, int32_t keycode) {
#pragma EXPORT_FUNC
