 ...
 * TLN_SetSpriteSet()
 */
bool TLN_SetSpritePicture(int nsprite, TLN_Tileset tileset, int entry) {
#pragma EXPORT_FUNC
  Sprite *sprite;
  if (nsprite >= engine->numsprites) {
    TLN_SetLastError(TLN_ERR_IDX_SPRITE);
  ...
  sprite->info.w = TLN_GetTileWidth(tileset);
  sprite->info.h = TLN_GetTileHeight(tileset);

  UpdateSprite(sprite);
  tln_trace(TLN_LOG_VERBOSE, "SetSpritePicture %d -> %d\n", nsprite, entry);
 ...
 */
bool TLN_SetSpritePalette(int nsprite, TLN_PaletteId palette_id) {
#pragma EXPORT_FUNC
  Sprite *sprite;
  if (nsprite >= engine->numsprites) {
    TLN_SetLastError(TLN_ERR_IDX_SPRITE);
 ...
 */
TLN_PaletteId TLN_GetSpritePalette(int nsprite) {
#pragma EXPORT_FUNC
  if (nsprite >= engine->numsprites) {
    TLN_SetLastError(TLN_ERR_IDX_SPRITE);
    return 0;
  ...
  TLN_SetLastError(TLN_ERR_OK);
  return engine->sprites[nsprite].palette_id;
}
...
#pragma EXPORT_FUNC
  ...
  TLN_SetLastError(TLN_ERR_OK);
  return engine->sprites[nsprite].tileset_entry;
}
...
bool TLN_EnableSprite(int nsprite) {
#pragma EXPORT_FUNC
  Sprite *sprite;
  if (nsprite >= engine->numsprites) {
    TLN_SetLastError(TLN_ERR_IDX_SPRITE);
...
#pragma EXPORT_FUNC
  ...
  state->index = sprite->tileset_entry;
